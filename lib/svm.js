...
var svmjs = (function(exports){
  ...
  */
  ...
  SVM.prototype = {
	...
	/**
	 * training with a different format of dataset. 
	 * @param dataset an array with objects of the format: {input: sample1, output: [0/1]}, ...
	 */
	trainBatch: function(dataset, options) {
		var data = [];
		var labels = [];
		...
		return this.train(data, labels, options);
	},
	
	trainOnline: function(dataset, options) {
		throw new Error("SVM does not support online training");
	},

	/**
    // this is the core prediction function. All others are for convenience mostly
    // and end up calling this one somehow.
    // @param inst - an array of length D. 
    // @return margin of given example
	*/
	...
	/**
    // Explain the classification of the given instance.
    // @param inst - an array of length D.
    // @param length - the max number of items in the explanation. 
    // @return an array with 'explanations' -- features and weights, ordered by their importance.
	*/
    explain: function(inst, length) {
        var f = this.b;
        // if the linear kernel was used and w was computed and stored,
        // (i.e. the svm has fully finished training)
        // the internal class variable usew_ will be set to true.
        ...
        return explanations.slice(0, length);
    },
    ...
    // classify is similar to predictOne, but it returns 0 for the negative class, instead of -1.
    // This is for consistency with other classifiers.
    classify: function(inst) {   
      return this.marginOne(inst) > 0 ? 1 : 0; 
    },
